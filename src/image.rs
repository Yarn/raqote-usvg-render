...
pub fn draw(
    ...
    match image.kind {
        usvg::ImageKind::SVG(ref tree) => {
            draw_svg(tree, image.view_box, opt, dt);
        }
        ref img_kind => {
            draw_raster(&img_kind, image.view_box, image.rendering_mode, opt, dt);
        }
    }
...
pub fn draw_raster(
    ...
    let img = try_opt!(load_raster(data, opt));
...
pub fn load_raster(
    ...
    let img = _load_raster(data, opt);

    if img.is_none() {
        warn!("Failed to load an image. ???");
    }
