...
fn render_svg(args: Args, tree: &usvg::Tree, out_png: &path::Path) -> Result<(), String> {
    let opt = raqote_usvg_render::Options {
        usvg: args.usvg,
        fit_to: args.fit_to,
    ...
    let img = if let Some(ref id) = args.export_id {
        if let Some(node) = tree.root().descendants().find(|n| &*n.id() == id) {
            timed!(args, "Rendering", raqote_usvg_render::render_node_to_image(&node, &opt))
        } else {
            return Err(format!("SVG doesn't have '{}' ID", id));
        }
    } else {
        timed!(args, "Rendering", raqote_usvg_render::render_to_image(&tree, &opt))
    };
...
const HELP: &str = "\
raqote-usvg-render is an SVG rendering application.

USAGE:
    raqote-usvg-render [OPTIONS] <in-svg> <out-png>

    raqote-usvg-render in.svg out.png
    raqote-usvg-render -z 4 in.svg out.png
    raqote-usvg-render --query-all in.svg
